   ...
   public let eventPublisher = PassthroughSubject<[URL], Never>()
    ...
    private func playAudioChunk(_ audioBuffer: MLXArray) {
        ...
        let outputURL = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("speech-test.wav")

        try? buffer.saveToWavFile(at: outputURL)
    ...
    private func saveAudioChunk(_ audioBuffer: MLXArray) -> URL?{
        ...
        let outputURL = URL(fileURLWithPath: NSTemporaryDirectory())
            ...
            .appendingPathComponent("speech-test\(scheduledBufferCount)1.wav")

        try? buffer.saveToWavFile(at: outputURL)
        print("Wav file saved to: \(outputURL.path)")
        print(buffer.frameLength)
